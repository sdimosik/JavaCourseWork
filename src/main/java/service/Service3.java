...
public class Service3 {
    ...
    private final ArrayList<Integer> lenQueueOutOfTime = new ArrayList<>(3);
    ...
    private TYPES_CARGO getNeededType() {
        int type = 0;
        ...
        TYPES_CARGO res;
        if (type == 0) {
            res = TYPES_CARGO.LOOSE;
        } else if (type == 1) {
            res = TYPES_CARGO.LIQUID;
        } else {
            res = TYPES_CARGO.CONTAINER;
        }
        return res;
    }
    ...
    private void fillSetting(ArrayList<Integer> countCrane) {
        ...
        for (int type = 0; type < 3; type++) {
            for (int i = 0; i < countCrane.get(type); i++) {
                Crane w = new Crane(phaser, TYPES_CARGO.getType(type).orElse(TYPES_CARGO.LOOSE));
                pool.get(type).add(w);
            }
        }
    }
